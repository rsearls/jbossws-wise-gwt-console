...
public class ClientHelper implements Serializable {
   ...
   //---------- rls start
   public static ParamNode getParamNode(WSMethod wsMethod, WSDynamicClient client) {
      WiseNodeBuilder builder = new WiseNodeBuilder(client, true);
      TreeNodeImpl rootElement = new TreeNodeImpl();
      ParamNode rootParamNode = /*null; //*/ new ParamNode();
      Collection<? extends WebParameter> parameters = wsMethod.getWebParams().values();
      SOAPBinding soapBindingAnn = wsMethod.getEndpoint().getUnderlyingObjectClass().getAnnotation(SOAPBinding.class);
      boolean rpcLit = false;
      if (soapBindingAnn != null) {
      ...
      for (WebParameter parameter : parameters) {
         if (parameter.getMode() != WebParam.Mode.OUT) {
            ...
            **/
            ParamNode wte = builder.buildTreeFromType(parameter.getType(),
               parameter.getName(), null, !rpcLit);
            //- rls test
            System.out.println("## WiseTreeElement: " + wte.getClass().getName());
            System.out.println("#### name: " + wte.getName() /*+ "  type: " + wte.getType()
            ...
            if (rootParamNode == null) {
               ...
               rootParamNode.addChild(wte);
            }

            /*************
            if (wte instanceof GroupWiseTreeElement) {
                 //- java.util.List<java.lang.String>
            } else if (wte instanceof EnumerationWiseTreeElement) {
               Map<String, String> eValuesMap = ((EnumerationWiseTreeElement)wte).getValidValue();
               if (eValuesMap != null) {
                  Set<String> eSet = eValuesMap.keySet();
                  for(String s : eSet) {
               ...
               pNode.setName(wte.getName());
               String tmpStr = wte.getClassType().toString();
               int index = tmpStr.lastIndexOf(" ");
               if (index > -1) {
                  pNode.setClassType(tmpStr.substring(index + 1));
               } else {
                  pNode.setClassType(tmpStr);
               }
               // pNode.setClassType(tNode.getClassType().toString());
               rootParamNode.addChild(pNode);
               System.out.println("@++@ wte pNode name: " + pNode.getName()
                  + "   pNode classType:" + pNode.getClassType() + "@++@");
            }
            ...
            if (keys != null) {
               while (keys.hasNext()) {
                  ...
                  if (tNode != null) {
                     ...
                     ParamNode pNode = new ParamNode();
                     pNode.setName(tNode.getName());
                     String tmpStr = tNode.getClassType().toString();
                     int index = tmpStr.lastIndexOf(" ");
                     if (index > -1) {
                        pNode.setClassType(tmpStr.substring(index + 1));
                     } else {
                     ...
                     // pNode.setClassType(tNode.getClassType().toString());
                     rootParamNode.addChild(pNode);
                     System.out.println("@++@ pNode name: " + pNode.getName()
                        + "   pNode classType:" + pNode.getClassType() +"@++@");
      ...
      //-return rootElement;
      dumpNode(rootParamNode);
      return rootParamNode;
   }

   private static void dumpNode (ParamNode rootNode) {
      System.out.println("dbg: name: " + rootNode.getName() + "  classType: " + rootNode.getClassType());
      if (rootNode instanceof ParameterizedNode) {
         ParameterizedNode pNode = (ParameterizedNode) rootNode;
         System.out.println("dbg: rawType: " + pNode.getRawType());
      }

      for(ParamNode p: rootNode.getChildren()) {
         dumpNode(p);
      }
   }
   //---------- rls end
